    ...
    <el-dialog :title="dialogCourseTitle" :visible.sync="dialogVisible"
      ...
      <div>
        <el-form :model="formDate" :rules="formRules" ref="formDate">
          ...
          <el-form-item label="编辑名称" :label-width="formLabelWidth" prop="name">
...
export default{
  ...
  methods: {
    ...
    saveFormData(){
      ...
      this.$refs['formDate'].validate(valid => {
        ...
        if (this.formDate.id || this.formDate.id === 0){ // 修改
          axios.put('/api/score/scCourse/update',this.formDate).then((resp) => {
            console.log("-----------",resp.data)
            if (resp.data.code === '200') {
              this.$message({
          ...
          axios.post('/api/score/scCourse/add', this.formDate).then((resp) => {
            console.log("-----------",resp.data)
            if (resp.data.code === '200') {
              this.$message({
                message: resp.data.description,
    ...
    openEditWindow(id){
      ...
      axios.get(`/api/score/scCourse/get/${id}`).then((resp) => { // 请求的是后端的接口 /api/score/scTeacher/1 1是传入的id
